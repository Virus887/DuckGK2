...
float4 main(PSInput i) : SV_TARGET
{
	...
	for (int k = 0; k < 2; k++)
	{
		...
		//float3 kd = (1.0f - fresnel(F0, norm, l)) * (1.0f - metallic);
	...
	float3 R = reflect(-i.view, N);
	float3 Ii = pfEnvMap.SampleLevel(samp, R, roughness.x * 6.0f).rgb;
	float2 brdf2 = brdfTex.Sample(samp, float2(max(dot(N, i.view), 0.0f), roughness.x)).rg;	//r -> fm, g-> f0
